 ...
 *   1) SITE_REVALIDATE_URL (لو محتاج رابط داخلي مختلف)
 *   2) NEXT_PUBLIC_SITE_URL (الرابط العام للموقع على اللايف)
...
/** بنبلّغ الموقع الأساسي إن الإعدادات اتغيرت (وبنتجاهل الفشل بهدوء) */
async function revalidatePublicSite() {
  try {
    await fetch(`${PUBLIC_SITE_URL}/api/revalidate-settings`, {
      method: "POST",
      headers: {

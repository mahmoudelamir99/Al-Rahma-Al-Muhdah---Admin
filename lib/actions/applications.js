...
import { requirePermissionAction, requireSuperAdminContext } from "@/lib/rbac";
...
/* ==========================================================================
   نظام الأرشيف (Soft Delete) + الاستعادة + الحذف النهائي
   --------------------------------------------------------------------------
   الحذف العادي بيبقى "ناعم": بنكتب deleted_at بدل ما الصف يتمسح، فيروح
   الأرشيف ويقدر أي موظف يستعيده. الحذف النهائي (مسح الصف من القاعدة)
   مخصص للمدير العام بس بأمانة، والتحقق بيتعمل على السيرفر مش في الواجهة.
   ========================================================================== */

/** حذف ناعم — محمي بصلاحية "تعديل طلبات التوظيف" */
export async function softDeleteApplication(id) {
  const auth = await requirePermissionAction("applications", "update");
  if (!auth.ok) return { ok: false, error: auth.error };
  if (!id) return { ok: false, error: "معرّف الطلب غير موجود." };
  ...
  try {
    const { data, error } = await getSupabaseAdmin()
      .from(APPLICATIONS_TABLE)
      .update({ deleted_at: new Date().toISOString(), deleted_by: auth.user.email || null })
      .eq("id", id)
      .is("deleted_at", null) // مينفعش نحذف طلب محذوف بالفعل
      .select("id, deleted_at")
      .maybeSingle();

    if (error) {
      if (error.code === "42703") {
        return { ok: false, error: "عمود الأرشيف مش موجود. شغّل ملف supabase/applications.sql من Supabase → SQL Editor." };
      }
      return { ok: false, error: `تعذّر نقل الطلب للأرشيف: ${error.message}` };
    }

    if (!data) return { ok: false, error: "الطلب مش موجود أو اتحذف بالفعل." };

    revalidatePath("/dashboard/applications");
    revalidatePath("/dashboard/applications/archive");
    revalidatePath("/dashboard");
    return { ok: true, message: "تم نقل الطلب للأرشيف." };
  } catch (err) {
    return { ok: false, error: `حصل خطأ غير متوقع: ${err?.message || err}` };
  }
}

/** استعادة من الأرشيف — أي موظف عنده صلاحية عرض/تعديل الطلبات يقدر يرجعها */
export async function restoreApplication(id) {
  ...
  try {
    const { data, error } = await getSupabaseAdmin()
      ...
      .eq("id", id)
      .not("deleted_at", "is", null) // لازم يكون محذوف عشان نستعيده
      .select("id, deleted_at")
      .maybeSingle();
    ...
    if (error) {
      ...
      return { ok: false, error: `تعذّر استعادة الطلب: ${error.message}` };
    }

    if (!data) return { ok: false, error: "الطلب مش موجود في الأرشيف." };

    revalidatePath("/dashboard/applications");
    revalidatePath("/dashboard/applications/archive");
    revalidatePath("/dashboard");
    return { ok: true, message: "تم استعادة الطلب للقائمة الرئيسية." };
  } catch (err) {
    return { ok: false, error: `حصل خطأ غير متوقع: ${err?.message || err}` };
  }
}
...
/**
 * حذف نهائي (Hard Delete) — مسح الصف من قاعدة البيانات.
 * 🔒 محمي مرتين: (1) requireSuperAdminContext على السيرفر، (2) الواجهة
 * بتخفي الزرار عن غير المدير العام. حتى لو حد نادى الدالة مباشرةً
 * من غير ما يشوف الزرار، السيرفر هيرفضه.
 */
export async function hardDeleteApplication(id) {
  ...
  try {
    ...
    if (error) return { ok: false, error: `تعذّر الحذف النهائي: ${error.message}` };
    if (!data || data.length === 0) return { ok: false, error: "الطلب مش موجود (ممكن يكون اتحذف نهائيًا من مكان تاني)." };

    revalidatePath("/dashboard/applications");
    revalidatePath("/dashboard/applications/archive");
    revalidatePath("/dashboard");
    return { ok: true, message: "تم حذف الطلب نهائيًا." };
  } catch (err) {
    return { ok: false, error: `حصل خطأ غير متوقع: ${err?.message || err}` };
  }
}

...
/*
 * شكل معرّف الوظيفة في Supabase هو **UUID** (نص) مش رقم — يعني حاجة زي
 * "687d18aa-f039-4e32-ade8-289ff9cafd61".
 *
 * 🐛 الإصلاح: قبل كده كانت الدالة بتشترط إن الـ id رقم موجب، فكانت بترجّع
 * null لكل وظيفة حقيقية — والنتيجة إن التعديل والحذف كانوا بيفشلوا برسالة
 * "معرّف الوظيفة غير صحيح — لازم يكون رقم موجب". دلوقتي بنتحقق إنه UUID صحيح.
 *
 * 🛑 مهم جداً: لسه بنرفض `undefined`/`null`/الفاضي برضه. لو الـ id وصل
 * `undefined` أو ""، استعلام `.eq("id", ...)` ممكن يتجاهل الشرط من الأساس —
 * وساعتها بيحصل UPDATE/DELETE على **كل صفوف الجدول** (mass update). حاجز
 * التحقق ده بيمنع الكارثة دي من المصدر. (القيمة لازم تكون UUID صح، و" مش UUID).
 */
...
export async function updateJob(id, input) {
  ...
  if (jobId === null) {
    return { ok: false, error: "معرّف الوظيفة غير صحيح." };
  }

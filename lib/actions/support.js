...
/* ==========================================================================
   ...
   ========================================================================== */
export async function requestPasswordChange({ currentPassword, newPassword, confirmPassword, message } = {}) {
  const auth = await requireAdmin();
  ...
  // ✅ التحقق إن الباسورد القديم صح — ده شرط أساسي قبل تسجيل الطلب
  const supabase = await createSupabaseServerClient();

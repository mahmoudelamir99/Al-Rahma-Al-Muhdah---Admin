...
/**
 * ⚡ الأداء: الدالة دي بتتحقق من الجلسة (نداء شبكة لـ Supabase) وبتقرا جدول
 * الموظفين (نداء تانٍ) — وكانت بتتنادى مرتين في نفس تحميل الصفحة: مرة في
 * `dashboard/layout.js` ومرة في `dashboard/page.js`، وكمان عند كل نداء لـ
 * requirePermission. يعني تحميل واحد = 4 رحلات شبكة زايدة بدون داعي.
 *
 * `cache()` من React بتخزّي نتيجة النداء الواحد جوه نفس الـ request، فأول
 * نداء بيعمل الشغل والباقي بياخد نفس النتيجة فورًا. كده التنقّل بين الصفحات
 * بقى أسرع بفرق حسي، والأمان ما اتغيرش (لسه بنتحقق من السيرفر).
 */
export const getCurrentAdminContext = cache(async function getCurrentAdminContext() {
  ...
  /*
   * 🐛 رسالة الإيقاف الواضحة:
   * قبل كده كان بيرجع error نصي عادي، والـ layout كان بيعمل redirect للشاشة
   * الرئيسية من غير أي سبب ظاهر — فالموظف الموقوف يتفضل يدخل ويطلع من غير
   * أي فهم. دلوقتي بنرجع flag `suspended` مستقل، والـ layout بيعرضه رسالة
   * مفهومة: "تم إيقاف حسابك، راجع الإدارة".
   */
  if (!admin.is_active) {
    return { ok: false, suspended: true, error: "تم إيقاف حسابك، راجع الإدارة." };
  }
